...
{
    public class IEDocument : DomContainer
    {
        private IntPtr HWND;

        public IEDocument(IntPtr hWND)
        {
            HWND = hWND;
        }
        ...
        public override INativeDocument OnGetNativeDocument()
        {
            try
            {
                return new WatiN.Core.Native.InternetExplorer.IEDocument(IEDOMFromhWnd(HWND));
            }
            catch (Exception)
            {
                return null;
        ...
        private IHTMLDocument2 IEDOMFromhWnd(IntPtr hWND)
        {
            ...
            if (IsIEServerWindow(hWND))
            {
                ...
                if (lRes != 0)
                {
                    ...
                    hr = IEUtils.ObjectFromLresult(lRes, ref IID_IHTMLDocument2, 0, ref ieDOMFromhWnd);
                    if (hr != 0)
                    {
