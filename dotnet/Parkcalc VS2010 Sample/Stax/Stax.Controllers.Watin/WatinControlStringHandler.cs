...
{
    public class WatinControlStringHandler
    {
        private DbConnectionStringBuilder _ContolStringBuilder;
        ...
        public string HTMLDialog { get; private set; }
        public string TableDefinition { get; private set; }
        public bool IsInReportViewer { get; private set; }
        public WatiN.Core.Constraints.Constraint WatinAttribute { get; private set; }

        // The order of strings in this array is important.
        // It determines the precedence of how attributes are used to find HTML elements.
        private string[] supportedHTMLAttributes = { "id", "name", "value", "alt", "custom", "for", "index", "src", "href", "style", "text", "title", "url", "innertext", "outertext", "sourceindex", "contentid" };
        ...
        /// <summary>
        /// Evaluates the and constructs a new  WatiN AttributeConstraint.
        /// </summary>
        private void EvaluateAndConstructWatiNAttribute()
        {
            if (_ContolStringBuilder.ContainsKey("CSS"))
            {
                CSSSelector = Convert.ToString(_ContolStringBuilder["CSS"]);
            }
            else if (_ContolStringBuilder.ContainsKey("css"))
            {
                CSSSelector = Convert.ToString(_ContolStringBuilder["css"]);
            }
            else
            {
                ...
                foreach (string item in supportedHTMLAttributes)
                {
                    // For each of the items in the array, try and construct a
                    // Watin AttributeConstraint. Once we are successfull stop evaluating;
                    if (ConstructWatinAttribute(item))
                    {
                        break;
                    }

                    // do as for each on the _ContolStringBuilder.Keys
                    //     Match all the ones that aren't special and add them as watinConstraints
                    //
                    // yay multiple constraint support :-)
